		...
		<view v-if="listdat" style="padding-bottom: 80px;">
			<view v-if="chooseItem_name == '血压'">
										...
										<view>
											<view
												...
												<view class="title_type_bg">
													<view v-if="xueyalist == 0" class="xueya_type"
														style="background: #58BF78;"></view>
													<view v-else-if="xueyalist == 1" class="xueya_type"
														style="background: #7AE545;"></view>
													<view v-else-if="xueyalist == 2" class="xueya_type"
														style="background: #FCCD41;"></view>
													<view v-else-if="xueyalist == 3" class="xueya_type"
														style="background: #F55A5A;"></view>
													<view v-else-if="xueyalist == 4" class="xueya_type"
														style="background: white;"></view>

													<view class="title_font_bg">{{level}}</view>
											...
											<view class="icon_item_bg">
												...
												<view>
													<view
														style="display: flex;flex-direction: row;justify-content: space-between; align-items: center;">
														<view class="title_type_bg">
															<view class="xueya_type"
																:style="bg(item.object.summary.lowPressureAvg,item.object.summary.highPressureAvg)" />
															<view class="title_font_bg">{{level}}</view>
															<uni-icons type="help" size="15" style=""
																@tap="BMI_tap"></uni-icons>
														</view>
				...
				<view v-else-if="act1 == true">
					...
					<view v-if="more_textma"
						style="display: flex;justify-content: center;margin-top: 20px;font-size: 16px;" @tap="more">
						查看更多
					</view>
					<view v-else style="display: flex;justify-content: center;margin-top: 20px;font-size: 16px;">
						没有更多数据了
					</view>
									...
									<view class="icon_item_bg">
										...
										<view>
											<view
												style="display: flex;flex-direction: row;justify-content: space-between; align-items: center;">
												<view class="title_type_bg">
													<view v-if="tizhilists == 0" class="xueya_type"
														style="background: #FCCD41" />
													<view v-else-if="tizhilists == 1" class="xueya_type"
														style="background: #58BF78" />
													<view v-else-if="tizhilists == 2" class="xueya_type"
														style="background: #FC7F41" />
													<view v-else-if="tizhilists == 3" class="xueya_type"
														style="background: #F55A5A" />
													<view v-else-if="tizhilists == 4" class="xueya_type"
														style="background: #7A0101" />
													<view v-else-if="tizhilists == 5" class="xueya_type"
											...
											<view class="icon_item_bg">
												...
												<view>
													<view
														...
														<view class="title_type_bg">
															<view class="xueya_type"
																:style="bg_tizhi(item.object.summary.bmiAvg)" />
															<view class="title_font_bg">{{level}}</view>
															<uni-icons type="help" size="15" style=""
																@tap="BMI_tap1"></uni-icons>
														</view>
	...
	export default {
		...
		methods: {
			...
			more() {
				...
				this.max1 += 1
				// this.queryDevices()
				// this.query_log_v2dsa(this.deviceSn, this.startTime, this
				// 	.endTime)
				console.log("的撒进看公告哈萨克", this.max)
				if (this.chooseItem_name == "血压") {
					this.query_log_v2(this.deviceSn, this.list_date1, this.list_date2)
				} else if (this.chooseItem_name == "体脂") {
					this.query_log_v22(this.deviceSn, this.list_date1, this.list_date2)
							...
							if (res.data.data == "") {
								...
								res.data.data.forEach((item, index) => {
									...
									if (that.act1 == true) {
										...
										if (res.data.data.length == (3 * that.max1)) {
											that.more_textma = false
										} else {
											if (that.max == 3) {
												that.swipeList = that.swipeList.slice(0, 3)
												that.more_textma = true
											} else {
												that.swipeList = that.swipeList.slice(0, 3 * that.max1)
												that.more_textma = true
											}
										}
									}
							...
							if (res.data.data.dataList == "") {
								...
								res.data.data.forEach((item, index) => {
									...
									if (res.data.data[index].object.summary.bmiAvg < 18.5) {
										...
										that.level = "正常体重"
										that.tizhilists = 1
									} else if (res.data.data[index].object.summary.bmiAvg >= 25.0 && res
										.data.data[index].object.summary.bmiAvg <= 29.9) {
										that.level = "超重"
										that.tizhilists = 2
									} else if (res.data.data[index].object.summary.bmiAvg >= 30.0 && res
										.data.data[index].object.summary.bmiAvg <= 34.9) {
										that.level = "一级肥胖"
										that.tizhilists = 3
									} else if (res.data.data[index].object.summary.bmiAvg >= 35.0 && res
										.data.data[index].object.summary.bmiAvg <= 39.9) {
										that.level = "二级肥胖"
										that.tizhilists = 4
									} else if (res.data.data[index].object.summary.bmiAvg >= 40) {
										that.level = "三级肥胖或病态肥胖"
										that.tizhilists = 5
									} else {
										that.level = "未知"
										that.tizhilists = 6
