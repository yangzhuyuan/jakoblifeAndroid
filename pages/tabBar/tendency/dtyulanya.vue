			...
			<view id="render-dom" class="render-content">
				...
				<view class="txtetitlestyle">
					<view class="texttitle">
						...
						<view class="text2style">{{pacidata}}</view>
					</view>
					<view class="texttitle">
				...
				<view class="charts-box pdf-keep-together">
					<qiun-data-charts type="line" canvasId="dtyulanChart" :opts="opts" :chartData="chartData"
						:ontouch="false" :animation="false" />
				</view>
				<view class="txtetitlestyle">
					<view style="font-size: 14px;font-weight: 600;color: #3298F7;">{{$t("")}}</view>
				</view>
				<view class="txtetitlestyle_1">
					<view class="pdf-keep-together pdf-table-head">
					...
					<view class="listitemall">
						...
						<view class="listitemall_2">
							<view class="itemall yejian-cell" :style="getblackgroud(yejian_1)">
								<text class="yejian-num">{{yejian_SZY_H}}</text>
								<text v-if="yejian_SZY_type" class="yejian-type">{{yejian_SZY_type}}</text>
							</view>
							<view class="itemall_1 yejian-cell" :style="getblackgroud(yejian_2)">
...
<script>
	...
	export default {
		...
		onLoad(res) {
			this.pacidata = res.profDate.slice(0, 10)
		...
		methods: {
			...
			handleImage() {
				/**
				 * 使用 urlToBase64 / pathToBase64 将图片转换为 base64，
				 * 若图片不支持跨域则可能会报 toDataURL on HTMLCanvasElement 的错。
				 * App 端优先 pathToBase64（plus.io），失败再回退 urlToBase64。
				 */
				if (this.imagesReadyPromise) {
					return this.imagesReadyPromise
				}
				const toBase64 = (path) => {
					return pathToBase64(path).catch(() => urlToBase64(path))
				}
				const lan = uni.getLocale();
				const tasks = [
					toBase64(lan == 'zh-Hans' ? '/static/splsh.png' : '/static/icons/loginssss.png')
					.then((res) => {
						this.appicon = res
					}),
					toBase64('/static/image/mb.png').then((res) => {
						this.maibo = res
					}),
					toBase64('/static/image/dtx.png').then((res) => {
						this.dtx = res
					}),
					toBase64('/static/icons/soudong.png').then((res) => {
						this.soudong = res
					}),
					toBase64('/static/icons/wuxiao.png').then((res) => {
						this.wuxiao = res
					}),
				]
				this.imagesReadyPromise = Promise.all(tasks)
					.then(() => {
						this.imagesReady = true
					})
					.catch((err) => {
						console.error('handleImage error:', err)
						this.imagesReadyPromise = null
					})
				return this.imagesReadyPromise
			},
			...
			get_retVarList(deviceSn) {
				let that = this
				const reportDay = String(that.pacitime || '').slice(0, 10)
				let data = {
					userId: uni.getStorageSync("userid"),
					profDate: reportDay + " 00:00:00",
				...
				that.$post(that.$url_APP_IP + "/prod-api/device_app/get_retVarList", data, {
					...
					if (get_retVarList.code === 200) {
						...
						for (let i = 0; resultArray.length > i; i++) {
							...
							seen.add(fullTime);
							if (that.idsss === "1") {
								that.chartData.categories.push(fullTime.slice(10, 13) + that.$t(
									"时"))
							} else {
								that.chartData.categories.push(fullTime.slice(8, 10) + that.$t(
									"日"))
							}
			...
			buildListallFromRetVarList(retVarListStr) {
				...
				for (const row of rows) {
					...
					if (date === reportDay && that.isTimeAfterSeven(time)) {
						item.rowBlue = true
					}
