...
<script>
	// +----------------------------------------------------------------------
	// | name:权限申请目的插件，主要用于解决华为、小米等上架问题 
	// +----------------------------------------------------------------------
	// | wechat:yangkang1200 
	// +----------------------------------------------------------------------
	// | Author: 月上柳梢 
	...
	export default {
		...
		setup(props, {emit}) {
			...
			//权限检测
			const requestPermissions = (permissionID) => {
				let _permissionID;
				// #ifdef APP-PLUS
				//判断安卓与ios设备
				if (plus.os.name == 'Android') {
					...
					if (permissionID == 'WRITE_EXTERNAL_STORAGE') {
						if (uni.getSystemInfoSync().brand == 'huawei' || uni.getSystemInfoSync().brand == 'xiaomi') {
							//华为跟小米获取相册使用WRITE_EXTERNAL_STORAGE权限
							_permissionID = 'android.permission.' + permissionID
						} else {
							//其他品牌手机先做安卓系统版本判断，分别使用不用的权限进行判断
							if (uni.getSystemInfoSync().osAndroidAPILevel >= 33) {
								_permissionID = 'android.permission.READ_MEDIA_IMAGES'
							} else {
								_permissionID = 'android.permission.READ_EXTERNAL_STORAGE'
							}
						}
					} else {
						_permissionID = 'android.permission.' + permissionID
					...
					if (permissionID == 'ACCESS_FINE_LOCATION') {
						...
						var authStatus = aVAudio.recordPermission()
